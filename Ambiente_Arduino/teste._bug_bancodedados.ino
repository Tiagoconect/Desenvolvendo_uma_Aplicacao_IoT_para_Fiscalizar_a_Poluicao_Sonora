...
void loop() {
  ...
  */
  ...
  if (tempo_decorrido > amostra) {  // Se o tempo decorrido for maior que a amostra, imprima o valor médio
   ...
   enviarInformacoesParaFirebase(tempo_atual, valor_medio, pico_maximo,frequencia_picos, anguloServo);
...
void enviarInformacoesParaFirebase(unsigned long tempo_atual, int valor_medio, int pico_maximo, float frequencia_picos, int anguloServo) {
  // Criar um novo objeto FirebaseJson para a nova amostra
  FirebaseJson json;

  // Preencher o objeto com os dados da amostra atual
  json.set("Tempo", tempo_atual);
  json.set("ValorMédio", valor_medio);
  json.set("PicoMáximo", pico_maximo);
  json.set("Frequência", frequencia_picos);
  json.set("Posição", anguloServo);

  // Enviar o novo objeto JSON para o Firebase
  String path = "/Amostras/" + String(tempo_atual); // Use a data/hora como o nome do documento
