...
/*=============================================MICROFONE-LEITURA DE DBS================================================================*/
  ...
  //int valor_Digital = digitalRead(OUT_PIN);// Ler o valor do microfone (entrada digital)

  

 // if (digitalRead(OUT_PIN) == LOW) {  // se sensor estiver captando (nivel logico alto) então guarda a amostra
    ...
    */
      ...
      //guarda_amostra++; // Incrementação confome o nivel logico alto


  //}

  /*if (tempo_decorrido > amostra) {  // se a o tempo decorrido for maior que amosta então imprima o valor acumulado
    Serial.println(guarda_amostra);  //imprimir o volor acumulado no monitor serial (Para teste inicial)

    
      float dBValor = converte_DB(guarda_amostra); // conversão 
        Serial.println(dBValor); //imprimir valor convertido
    
    
    guarda_amostra = 0; // então limpa a amostra 
   tempo_anterior =  tempo_atual ; // então tempo anterior recebe tempo atual
  }*/
